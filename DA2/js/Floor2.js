...
GameStates.makeFloor2 = function( game, shared ) {
	...
	function setUpPuzzle(){
		...
		card1[0] = game.add.sprite(game.camera.x + 200, 150, 'card1');
		card1[1] = game.add.sprite(game.camera.x + 200, 300, 'card1');
		card2[0] = game.add.sprite(game.camera.x + 500, 150, 'card2');
		card2[1] = game.add.sprite(game.camera.x + 50, 300, 'card2');
		card3[0] = game.add.sprite(game.camera.x + 50, 150, 'card3');
		card3[1] = game.add.sprite(game.camera.x + 650, 150, 'card3');
		card4[0] = game.add.sprite(game.camera.x + 350, 150, 'card4');
		card4[1] = game.add.sprite(game.camera.x + 500, 300, 'card4');
		card5[0] = game.add.sprite(game.camera.x + 350, 300, 'card5');
		card5[1] = game.add.sprite(game.camera.x + 650, 300, 'card5');
		...
		for (var i = 0; i < 2; i++){
			...
			card4[i].inputEnabled = true;
			card4[i].animations.add('back', [0]);
			card4[i].animations.add('flip', [1]);
			card5[i].inputEnabled = true;
			card5[i].animations.add('back', [0]);
			card5[i].animations.add('flip', [1]);
		}
	}
		...
		for (var i = 0; i < 2; i++){
			if (canClick){
				if (card1[i].input.checkPointerDown(pointer)){
					if ((flip1 + flip2 + flip3 + flip4 + flip5) == 2){
				...
				if (card4[i].input.checkPointerDown(pointer)){
					if ((flip1 + flip2 + flip3 + flip4 + flip5) == 2){
						//	Only reach here if player was unsuccessful
						resetPuzzle();
					}
					//	Player chose card, flip it over and do not let them choose it again
					card4[i].play('flip');
					card4[i].inputEnabled = false;
					flip4++;
					canClick = false;
				}
				if (card5[i].input.checkPointerDown(pointer)){
					if ((flip1 + flip2 + flip3 + flip4 + flip5) == 2){
						//	Only reach here if player was unsuccessful
						resetPuzzle();
					}
